...
function PlayerModel:loadSounds(xmlFile, isOwner)
	...
	if not isOwner then
		...
		si.samples.swim = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.water", "swim", self.baseDirectory, self.rootNode, 0, AudioGroup.ENVIRONMENT, nil, nil)
		si.samples.swimIdle = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.water", "swimIdle", self.baseDirectory, self.rootNode, 0, AudioGroup.ENVIRONMENT, nil, nil)
		si.samples.plunge = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.water", "plunge", self.baseDirectory, self.rootNode, 1, AudioGroup.ENVIRONMENT, nil, nil)
		si.samples.flashlight = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.tools", "flashlight", self.baseDirectory, self.rootNode, 1, AudioGroup.ENVIRONMENT, nil, nil)
		si.samples.horseBrush = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.tools", "horseBrush", self.baseDirectory, self.rootNode, 1, AudioGroup.ENVIRONMENT, nil, nil)
		si.samples.jump = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.jump", "takeoff", self.baseDirectory, self.rootNode, 0, AudioGroup.ENVIRONMENT, nil, nil)
		si.samples.touchGround = g_soundManager:loadSampleFromXML(xmlFile, "player.sounds.jump", "landing", self.baseDirectory, self.rootNode, 0, AudioGroup.ENVIRONMENT, nil, nil)
	else
		for _, surfaceSound in pairs(g_currentMission.surfaceSounds) do
			if surfaceSound.type == "footstep" and surfaceSound.sample ~= nil then
