	...
	if code == VehicleLoadingUtil.VEHICLE_LOAD_OK then
		if vehiclesToLoad.storeItem.bundleInfo ~= nil then
			...
			for _, attachInfo in pairs(vehiclesToLoad.storeItem.bundleInfo.attacherInfo) do
				...
				v1:attachImplement(v2, attachInfo.inputAttacherJointIndex, attachInfo.attacherJointIndex, true, nil, nil, true)
			...
			if g_server ~= nil then
				g_server:broadcastEvent(VehicleBundleAttachEvent.new(bundleInfo), nil, nil, g_currentMission)
			end
		end
