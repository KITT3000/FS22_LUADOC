...
function ConstructionBrushFoliage:performBrush(isDown, isDrag, isUp, direction)
	...
	if direction > 0 then
		requestLandscaping = LandscapingSculptEvent.new(validateOnly, Landscaping.OPERATION.FOLIAGE, x, y, z, nil, nil, nil, nil, nil, nil, radius, 1, self.brushShape, Landscaping.TERRAIN_UNIT, nil, self.foliagePaint.id, self.foliageState)
	else
		requestLandscaping = LandscapingSculptEvent.new(validateOnly, Landscaping.OPERATION.PAINT, x, y, z, nil, nil, nil, nil, nil, nil, radius, 1, self.brushShape, Landscaping.TERRAIN_UNIT, TerrainDeformation.NO_TERRAIN_BRUSH)
	end
