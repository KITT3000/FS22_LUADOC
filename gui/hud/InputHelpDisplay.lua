		...
		if i <= #helpElements then
			...
			if helpElement.actionName ~= "" then
				if helpElement.actionName2 ~= "" then
					...
					}, nil, nil, helpElement.inlineModifierButtons)
				else
					self.inputGlyphs[i]:setAction(helpElement.actionName, nil, nil, helpElement.inlineModifierButtons)
				end
...
function InputHelpDisplay:onInputDevicesChanged()
	for _, combos in pairs({
		...
		for i, combo in ipairs(combos) do
			...
			glyphElement:setAction(actionName, nil, nil, false, true)
